...
function getDaysSince19000131(date: Date): number {
...
export function getLunarDate(date: Date): string {
  ...
  // 计算从1900年1月31日到目标日期的天数
  let daysSinceBase = getDaysSince19000131(new Date(year, month - 1, day));
  ...
  while (true) {
    ...
    let daysInYear = 354;
    if (isLeapYear) {
      daysInYear += 29; // 闰月为29天
    }
    
    if (daysSinceBase < daysInYear) {
      break;
    }
